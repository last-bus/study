...
if (getToken("username") && getToken("password")) {
  //判断是否存储过账户密码
  loginForm.value.username = decode(getToken("username"));
  loginForm.value.password = decode(getToken("password"));
}
removeToken("token");
...
const refLoginForm = ref(null); //表单ref
const handleLogin = () => {
  refLoginForm.value.validate((valid, fields) => {
    if (valid) {
      loading.value = true;
      let data = {
